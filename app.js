...
app.post('/details', (req, res) => {
    ...
    data.findOne({ disease: Disease, gender: Gender, severity: Severity, age: Age },{_id:false,drug:true})
        .then((medicine) => {
            if (medicine) {
                let medicinename=JSON.stringify(medicine);
                // console.log('datatype : '+typeof(medicinename));
                ...
                let params={'content':medicinename3};
                res.status(200).render('medicine.pug',params);
            }
            else 
            {
                let params={'content':'database has not been updated yet'};
                res.status(200).render('medicine.pug',params);
...
app.listen(port, hostname, () => {
    console.log(`The website is succesfully running at http://${hostname}:${port}/`);
})